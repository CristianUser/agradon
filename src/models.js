...
const fs = require('fs'),
  ...
  { getSchemas, getModels, getEntityName, toPascalCase } = require('./utils');
...
function loadSchemas() {
  return getSchemas().reduce((obj, schema) => {
    obj[getEntityName(schema)] = readSchemaFile(schema);
    return obj;
  }, {});
}
...
function getSchemaType(type) {
  ...
  if (_.isString(type) && TYPES[type.toLowerCase()]) {
