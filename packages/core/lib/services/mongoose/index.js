"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MongooseDB = void 0;
var lodash_1 = __importDefault(require("lodash"));
var mongoose_1 = __importDefault(require("mongoose"));
var files_1 = require("../files");
var log_1 = require("../log");
var log = (0, log_1.createLogger)({ file: __filename });
function getSchemaType(type) {
    var _a = mongoose_1.default.Types, Decimal128 = _a.Decimal128, ObjectId = _a.ObjectId;
    var TYPES = {
        string: String,
        boolean: Boolean,
        number: Number,
        date: Date,
        buffer: Buffer,
        objectid: ObjectId,
        array: Array,
        decimal128: Decimal128,
        map: Map,
        object: Object
    };
    if (typeof type === 'string' && TYPES[type.toLowerCase()]) {
        return TYPES[type.toLowerCase()];
    }
    if (Array.isArray(type)) {
        return lodash_1.default.flatten([type.map(function (t) { return TYPES[t.toLowerCase()]; })]);
    }
    return TYPES.string;
}
function parseSchemaField(schemaFile, fieldKey) {
    var field = lodash_1.default.get(schemaFile, fieldKey);
    if (lodash_1.default.get(field, 'type')) {
        if (field.type === 'array') {
            parseSchemaField(field, 'items');
            schemaFile[fieldKey] = [field.items];
        }
        else {
            lodash_1.default.set(schemaFile, "".concat(fieldKey, ".type"), getSchemaType(field.type));
        }
    }
    else if (lodash_1.default.get(field, '$ref')) {
        field.type = getSchemaType('ObjectId');
        field.ref = field.$ref.split('/').pop();
        delete field.$ref;
    }
    else if (typeof field === 'object') {
        for (var fieldKey1 in field) {
            parseSchemaField(field, fieldKey1);
        }
    }
}
function parseSchemaFields(jsonSchema) {
    for (var fieldKey in jsonSchema.properties) {
        parseSchemaField(jsonSchema.properties, fieldKey);
        if (jsonSchema.required.includes(fieldKey)) {
            jsonSchema.properties[fieldKey].required = "\"".concat(fieldKey, "\" is required\"");
        }
    }
}
function mapSchemasType(schemas) {
    for (var key in schemas) {
        var schemaFile = schemas[key];
        parseSchemaFields(schemaFile);
    }
    return schemas;
}
function setDefaultHooks(schema) {
    schema.set('toJSON', {
        transform: function (doc, ret) {
            ret.id = ret._id;
            delete ret._id;
            delete ret.__v;
        }
    });
}
function setSchemaHooks(schema, modelFile) {
    if (lodash_1.default.get(modelFile, 'schema') && typeof modelFile.schema === 'function') {
        var schemaResult = modelFile.schema(schema);
        if (schemaResult) {
            schema = schemaResult;
        }
        else {
            log.error("schema isn't been returned in one model.js");
        }
    }
    setDefaultHooks(schema);
}
function createMongooseSchema(_a, modelFile) {
    var properties = _a.properties;
    var newSchema = new mongoose_1.default.Schema(properties, { timestamps: true });
    setSchemaHooks(newSchema, modelFile);
    return newSchema;
}
var MongooseDB = (function () {
    function MongooseDB(mongoose) {
        this.repositories = {};
        this.type = 'mongoose';
        this.models = {};
        this.mongoose = mongoose;
    }
    MongooseDB.prototype.connect = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2, this.mongoose.connection.asPromise()];
            });
        });
    };
    MongooseDB.prototype.loadModels = function (filesSets) {
        var schemaFiles = (0, files_1.getFileGroup)(filesSets, 'schema');
        var modelFiles = (0, files_1.getFileGroup)(filesSets, 'model');
        var schemas = mapSchemasType(lodash_1.default.cloneDeep(schemaFiles));
        for (var key in schemas) {
            var schema = schemas[key];
            var model = modelFiles[key];
            this.models[key] = this.mongoose.model(key, createMongooseSchema(schema, model));
        }
        return this;
    };
    MongooseDB.prototype.getModel = function (name) {
        return this.models[name];
    };
    MongooseDB.prototype.getRepository = function (modelName) {
        return this.repositories[modelName];
    };
    return MongooseDB;
}());
exports.MongooseDB = MongooseDB;
exports.default = {};
//# sourceMappingURL=index.js.map